...
static void rb_zk_mutex_error_done(struct rb_zk *zk,
...
static int
override_leader_node(struct rb_zk_mutex *lock, const char *leader_node) {
	if (lock->path) {
		free(lock->path);
	}
...
static void leader_get_children_complete(int rc,
					 const struct String_vector *strings,
					 const void *data);

static void previous_leader_watcher(zhandle_t *zh,
				    int type,
				    int state,
				    const char *path,
				    void *_mutex) {
	...
	/* Something happens with previous leader. Check if we are the new
	 * master */
	struct rb_zk_mutex *mutex = monitor_zc_mutex_casting(_mutex);
	zoo_aget_children(mutex->context->handler,
			  buf,
			  0,
			  leader_get_children_complete,
			  _mutex);
}

/// Return minimum string and inmediate less than 'my_str' strings.
static void leader_previous_string(const struct String_vector *strings,
				   const char *my_str,
				   const char **bef_str) {
	size_t i = 0;
	...
	if (_my_str) {
		...
		rdlog(LOG_ERR,
		      "Can't find last '/' of our path [%s]. Exiting.",
		      my_str);
	}
	...
	for (i = 0; i < strings->count; ++i) {
		...
		if (NULL != _my_str && NULL != bef_str) {
			const int my_str_cmp_rc =
					strcmp(strings->data[i], _my_str);
			if (my_str_cmp_rc < 0 &&
			    (NULL == *bef_str ||
			     strcmp(strings->data[i], *bef_str) > 0)) {
...
static void leader_get_children_complete(int rc,
	...
	if (NULL == bef_str) {
		...
		snprintf(prev_node_path,
			 sizeof(prev_node_path),
			 "%s/%s",
			 buf_path,
			 bef_str);
		...
		struct Stat stat;
		rdlog(LOG_INFO,
		      "I'm not the leader of %s. Trying to watch %s",
		      mutex->path,
		      prev_node_path);
		const int wget_rc = zoo_wexists(mutex->context->handler,
						prev_node_path,
						previous_leader_watcher,
						mutex,
						&stat);
		if (wget_rc != 0) {
			rb_zk_mutex_error_done(mutex->context,
					       mutex,
					       wget_rc,
					       "Can't wget previous node");
		}
	}
}

static void create_mutex_node_complete(int rc,
				       const char *leader_node,
				       const void *_mutex) {
	char buf[BUFSIZ];
	struct rb_zk_mutex *mutex = monitor_zc_mutex_const_casting(_mutex);
	...
	if (aget_children_rc != 0) {
		rb_zk_mutex_error_done(mutex->context,
				       mutex,
				       rc,
				       "Couldn't aget children");
	}
}

static struct rb_zk_mutex *
rb_zk_mutex_create_entry(struct rb_zk *zk,
			 const char *leader_path,
			 rb_mutex_status_change_cb schange_cb,
			 rb_mutex_error_cb error_cb,
			 void *cb_opaque) {
	assert(zk);
...
static void rb_zk_mutex_lock0(struct rb_zk *zk, struct rb_zk_mutex *mutex) {
	...
	const int acreate_rc = zoo_acreate(zk->handler,
					   rb_zk_mutex_path(mutex),
					   ZOOKEEPER_LOCK_NODE_CONTENT,
					   strlen(ZOOKEEPER_LOCK_NODE_CONTENT),
					   &acl,
					   ZOO_EPHEMERAL | ZOO_SEQUENCE,
					   create_mutex_node_complete,
					   mutex);

	if (ZOK != acreate_rc) {
		rb_zk_mutex_error_done(
				zk, mutex, acreate_rc, "Can't call acreate");
	} else {
		rdlog(LOG_DEBUG,
		      "acreate called successfuly for path %s",
		      rb_zk_mutex_path(mutex));
	}
}
...
struct rb_zk_mutex *rb_zk_mutex_lock(struct rb_zk *zk,
	...
	struct rb_zk_mutex *_mutex = rb_zk_mutex_create_entry(
			zk, leader_path, schange_cb, error_cb, cb_opaque);
...
void rb_zk_mutex_unlock(struct rb_zk *zk, struct rb_zk_mutex *mutex) {
	...
	if (zoo_state(zk->handler) == ZOO_CONNECTED_STATE) {
		if (NULL == mutex->path) {
			rdlog(LOG_ERR,
			      "Does not seen mutex path -> cannot delete");
		} else {
			/// @TODO adelete error treatment
			zoo_adelete(zk->handler,
				    mutex->path,
				    -1,
				    delete_mutex_completed,
				    mutex);
		}
	} else {
		// mutex already unlocked, since locks are ephimerals -> free
		// resources
...
static void zk_watcher_clean_mutex(struct rb_zk *context, int type, int state) {
	...
	snprintf(buf,
		 sizeof(buf),
		 "ZK disconnected [type=%s][state=%s]",
		 type2String(type),
		 state2String(state));
	...
	rd_avl_init(&context->leaders_avl,
		    context->leaders_avl.ravl_cmp,
...
static struct rb_zk_queue_element *
rb_zk_queue_element_const_cast(const void *a) {
...
static const char *min_str(const struct String_vector *strings) {
	...
	for (i = 0; i < strings->count; ++i) {
		...
		if (NULL == _min_str ||
		    strcmp(strings->data[i], _min_str) < 0) {
...
static void rb_zk_queue_get_element(int rc,
				    const char *value,
				    int value_len,
	...
	if (rc < 0) {
		qelm->error_cb(qelm->rb_zk,
			       qelm,
			       "Error getting queue element",
			       rc,
			       qelm->opaque);
		rb_zk_queue_element_decref(qelm);
		return;
	...
	const int adelete_rc = zoo_adelete(qelm->rb_zk->handler,
					   qelm->queue_element_path,
					   IGNORE_NODE_VERSION,
					   rb_zk_queue_delete_completed,
					   data);
...
static void rb_zk_queue_child_watcher(zhandle_t *zh,
				      int type,
				      int state,
				      const char *path,
				      void *watcherCtx) {
	...
	struct rb_zk_queue_element *qelm =
			rb_zk_queue_element_const_cast(watcherCtx);
...
static void
rb_zk_queue_pop_minimum_no_watcher_setted(int rc,
					  const struct String_vector *strings,
					  const void *_data) {
...
static void
rb_zk_queue_pop_minimum_watcher_setted(int rc,
				       const struct String_vector *strings,
				       const void *_data) {
...
static void rb_zk_queue_pop_minimum0(int rc,
	...
	char buf[BUFSIZ];
	struct rb_zk_queue_element *qelm =
			rb_zk_queue_element_const_cast(_data);

	if (rc < 0) {
		qelm->error_cb(qelm->rb_zk,
			       qelm,
			       "Error getting queue minimum",
			       rc,
			       qelm->opaque);
		rb_zk_queue_element_decref(qelm);
		return;
	...
	if (NULL != _min_str) {
		...
		if (watcher_setted) {
			rdlog(LOG_DEBUG,
			      "Watcher already setted. Marking to ignore it.");
			qelm->should_ignore_watcher = 1;
		}

		rdlog(LOG_DEBUG,
		      "Trying to get sensor %s",
		      qelm->queue_element_path);
		const int aget_rc = zoo_aget(qelm->rb_zk->handler,
					     qelm->queue_element_path,
					     0,
					     rb_zk_queue_get_element,
					     qelm);
		if (aget_rc != 0) {
			qelm->error_cb(qelm->rb_zk,
				       qelm,
				       "Can't do aget over min element queue",
				       aget_rc,
				       qelm->opaque);
			rb_zk_queue_element_decref(qelm);
		}
		...
		parent_node(buf);
		rdlog(LOG_DEBUG,
		      "No more sensors to get, I will watch %s",
		      buf);
		zoo_awget_children(qelm->rb_zk->handler,
				   buf,
				   rb_zk_queue_child_watcher,
				   qelm,
				   rb_zk_queue_pop_minimum_watcher_setted,
				   qelm);
	} else {
		rdlog(LOG_DEBUG,
		      "Still no sensor to get. Waiting the watcher event.");
	}
}
...
void rb_zk_queue_pop_nolock(struct rb_zk *zk,
	...
	const int aget_children_rc = zoo_aget_children(
			zk->handler,
			buf,
			0,
			rb_zk_queue_pop_minimum_no_watcher_setted,
			qelement);
	if (aget_children_rc < 0) {
		qelement->error_cb(zk,
				   qelement,
				   "Can't call aget.",
				   aget_children_rc,
				   qelement->opaque);
...
static void
rb_zk_queue_mutex_status_change(struct rb_zk_mutex *mutex, void *opaque) {
...
static void rb_zk_queue_mutex_error_cb(struct rb_zk *rb_zk,
				       struct rb_zk_mutex *mutex,
				       const char *cause,
				       int rc,
				       void *opaque) {
...
void rb_zk_queue_pop(struct rb_zk *zk,
	...
	qelement->queue_mutex =
			rb_zk_mutex_lock(zk,
					 mutex_path,
					 rb_zk_queue_mutex_status_change,
					 rb_zk_queue_mutex_error_cb,
					 qelement);
}
...
void rb_zk_queue_push(struct rb_zk *zk,
		      ...
		      const char *value,
		      int valuelen,
		      string_completion_t create_mutex_node_complete,
		      void *opaque) {
	...
	if (ZOK != acreate_rc) {
		rdlog(LOG_ERR,
		      "Can't call acreate to push queue %s element, rc=%d",
		      path,
		      acreate_rc);
	} else {
		rdlog(LOG_DEBUG,
		      "acreate called successfuly for queue %s",
		      path);
	}
}
...
static void zk_watcher(zhandle_t *zh,
		       int type,
		       int state,
		       const char *path,
		       void *_context) {
	...
	if (type == ZOO_SESSION_EVENT && state == ZOO_CONNECTED_STATE) {
		...
		rd_thread_func_call1(context->zk_thread,
				     zk_watcher_do_pending_locks,
				     context);
	} else {
		rdlog(LOG_ERR,
		      "Can't connect to ZK: [type: %d (%s)][state: %d (%s)]",
		      type,
		      type2String(type),
		      state,
		      state2String(state));
...
int rb_zk_create_recursive_node(struct rb_zk *context,
				const char *node,
				int flags) {
	...
	while (cursor || !last_path_printed) {
		...
		if (create_rc != ZOK) {
			if (create_rc != ZNODEEXISTS)
				rdlog(LOG_ERR,
				      "Can't create zookeeper path [%s]: %s",
				      node,
				      zerror(create_rc));
		}
...
static void reset_zk_context(struct rb_zk *context) {
	...
	if (context->handler) {
		...
		if (close_rc != 0) {
			rdlog(LOG_ERR,
			      "Error closing ZK connection [rc=%d]",
			      close_rc);
		}
	}

	context->handler = zookeeper_init(context->zk_host,
					  zk_watcher,
					  context->zk_timeout,
					  0,
					  context,
...
struct rb_zk *rb_zk_init(char *host, int zk_timeout) {
	...
	if (NULL == _zk) {
		rdlog(LOG_ERR,
		      "Can't allocate zookeeper handler (out of memory?)");
		return NULL;
	}
